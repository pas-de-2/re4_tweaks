			...
			if (Tab == MenuTab::Gameplay)
			{
				if (ImGui::BeginTable("Gameplay", columnCount, ImGuiTableFlags_PadOuterX, ImVec2(ImGui::GetItemRectSize().x - 12, 0)))
				{
					...
					if ((OptionsFilter.PassFilter("NAGameCubeBalance NTSC") && OptionsFilter.IsActive()) || !OptionsFilter.IsActive())
					{
						...
						ImGui::TextWrapped("Emulates the game balance found in the original North American GameCube revision of RE4. "
							"Reverts weapon stats, merchant prices, and item drop rates. Automatically enables NTSC mode and RevertWiiKnifeBuff.");
						ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(0.51f, 0.00f, 0.14f, 0.00f));
